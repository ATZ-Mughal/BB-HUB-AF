...
async def start(client: Client, message):
    ...
    if len(message.command) == 2 and message.command[1].startswith("reff_"):
        ...
        if fromuse == 100:
            ...
            await message.reply_text(
                f"𝙔𝙤𝙪 𝙝𝙖𝙫𝙚 𝙗𝙚𝙚𝙣 𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙞𝙣𝙫𝙞𝙩𝙚𝙙 𝙗𝙮 {uss.mention}!"
            )
            await client.send_message(
                user_id,
                text=f"𝙔𝙤𝙪 𝙝𝙖𝙫𝙚 𝙗𝙚𝙚𝙣 𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙞𝙣𝙫𝙞𝙩𝙚𝙙 𝙗𝙮 {message.from_user.mention}!",
            )
            ...
            await client.send_message(
                user_id, f"𝙔𝙤𝙪 𝙝𝙖𝙫𝙚 𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙞𝙣𝙫𝙞𝙩𝙚𝙙 {message.from_user.mention}!"
            )
        return
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await aks.delete()
        await m.delete()
        if (str(message.chat.id)).startswith("-100") and not await db.get_chat(
            message.chat.id
        ):
            ...
            if refData and refData.split("-", 1)[0] == "Jisshu":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                    buttons = [
                        ...
                        [
                            InlineKeyboardButton(
                                "Pʀᴇᴍɪᴜᴍ 🎫", callback_data="seeplans"
                            ),
                            InlineKeyboardButton("Rᴇғᴇʀ ⚜️", callback_data="reffff"),
                        ],
                        [
                            InlineKeyboardButton(
                                "Mᴏsᴛ Sᴇᴀʀᴄʜ 🔍", callback_data="mostsearch"
                            ),
                            InlineKeyboardButton(
                                "Tᴏᴘ Tʀᴇɴᴅɪɴɢ ⚡", callback_data="trending"
                            ),
                        ],
                    ]
                    reply_markup = InlineKeyboardMarkup(buttons)
                    m = await message.reply_sticker(
                        "CAACAgUAAxkBAAIHwmebVHVqXWQAAefPCn6kSpe9omcDJwACaA4AAvjW2FewUZuR0mLtbTYE"
                    )
                ...
                try:
                    if newPoint == 0:
                        await client.send_message(
                            refUserId, script.REF_PREMEUM.format(PREMIUM_POINT)
                        )
                    else:
                        await client.send_message(
                            refUserId,
                            script.REF_START.format(
                                message.from_user.mention(), newPoint
                            ),
                        )
                except:
                    pass
    ...
    if AUTH_CHANNEL and not await is_req_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            ...
            try:
                chksub_data = (
                    message.command[1].replace("pm_mode_", "")
                    if pm_mode
                ...
                btn.append(
                    [
                        InlineKeyboardButton(
                            "♻️ ᴛʀʏ ᴀɢᴀɪɴ ♻️",
                            callback_data=f"checksub#{file_id}#{int(grp_id)}",
                        )
                    ]
                )
        ...
        if (
            ...
            buttons = [
                [
                    ...
                    InlineKeyboardButton(
                        text="ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ❓", url=settings["tutorial"]
                    ),
                ],
                [
                    InlineKeyboardButton(
    ...
    if data and data.startswith("allfiles"):
        ...
        delCap = "<b>ᴀʟʟ {} ғɪʟᴇs ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴀғᴛᴇʀ {} ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ᴠɪᴏʟᴀᴛɪᴏɴs!</b>".format(
            len(files_to_delete),
            (
        ...
        afterDelCap = "<b>ᴀʟʟ {} ғɪʟᴇs ᴀʀᴇ ᴅᴇʟᴇᴛᴇᴅ ᴀғᴛᴇʀ {} ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ᴠɪᴏʟᴀᴛɪᴏɴs!</b>".format(
            len(files_to_delete),
            (
                f"{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs"
                if FILE_AUTO_DEL_TIMER >= 60
    ...
    afterDelCap = (
        "<b>ʏᴏᴜʀ ғɪʟᴇ ɪs ᴅᴇʟᴇᴛᴇᴅ ᴀғᴛᴇʀ {} ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ᴠɪᴏʟᴀᴛɪᴏɴs!</b>".format(
            f"{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs"
            if FILE_AUTO_DEL_TIMER >= 60
            else f"{FILE_AUTO_DEL_TIMER} sᴇᴄᴏɴᴅs"
        )
    ...
    if settings is not None:
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    "ᴏɴ ✓" if settings["is_verify"] else "ᴏғғ ✗",
                    callback_data="verifyon",
                ),
            ],
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/Movie_Links_Hub</code>"
        )
    await save_group_settings(grp_id, "tutorial", tutorial)
    await message.reply_text(
        f"<b>Successfully changed tutorial for {title} to</b>\n\n{tutorial}",
        disable_web_page_preview=True,
    )
...
async def set_shortner(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply(
            "<b>Use this command like this - \n\n`/set_shortner onylinks.com 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>"
        )
        return
    ...
    try:
        ...
        API = m.command[2]
        resp = requests.get(
            f"https://{URL}/api?api={API}&url=https://telegram.dog/Movie_Links_Hub"
        ).json()
        ...
        await m.reply_text(
            f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/BB_HUB_Support>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner onylinks.com e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>",
            quote=True,
        )
...
async def set_shortner_2(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply(
            "<b>Use this command like this - \n\n`/set_shortner_2   onylinks.com 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>"
        )
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    ...
    try:
        ...
        await m.reply_text(
            f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/BB_HUB_Support>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 onylinks.com e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>",
            quote=True,
        )
...
async def set_shortner_3(c, m):
    ...
    if len(m.text.split()) == 1:
        return await m.reply(
            "<b>Use this command like this - \n\n`/set_shortner_3 onylinks.com 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>"
        )
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await sts.delete()
    ...
    if len(m.command) == 1:
        await m.reply_text(
            "<b>ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀᴅᴅ sʜᴏʀᴛɴᴇʀ & ᴀᴘɪ\n\nᴇx - `/set_shortner_3 onylinks.com e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`</b>",
            quote=True,
        )
        return
    try:
        ...
        API = m.command[2]
        resp = requests.get(
            f"https://{URL}/api?api={API}&url=https://t.me/BB_HUB_Support"
        ).json()
        if resp["status"] == "success":
            SHORT_LINK = resp["shortenedUrl"]
        ...
        await m.reply_text(
            f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/BB_HUB_Support>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_3 onylinks.com e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>",
            quote=True,
        )
...
async def refer(bot, message):
    btn = [
        [
            InlineKeyboardButton(
                "invite link",
                url=f"https://telegram.me/share/url?url=https://t.me/{bot.me.username}?start=reff_{message.from_user.id}&text=Hello%21%20Experience%20a%20bot%20that%20offers%20a%20vast%20library%20of%20unlimited%20movies%20and%20series.%20%F0%9F%98%83",
            ),
            InlineKeyboardButton(
...
async def generate_verify_id(bot, message):
    ...
    if grpid in verification_ids:
        await message.reply_text(
            f"An active Verify ID already exists for this group: `/verifyoff {verification_ids[grpid]}`"
        )
        return
    ...
    await message.reply_text(
        f"Verify ID: `/verifyoff {verify_id}` (Valid for this group, one-time use)"
    )
    return
...
async def verifyoff(bot, message):
    ...
    grpid = message.chat.id
    if not await is_check_admin(
        bot, grpid, message.from_user.id
    ):  # Changed client to bot
    ...
    try:
        ...
        return await message.reply_text(
            "Please provide the Verify ID along with the command.\nUsage: `/verifyoff {id}`"
        )

    if grpid not in verification_ids or verification_ids[grpid] != input_id:
        return await message.reply_text(
            "Invalid Verify ID! Please contact the admin for the correct ID."
        )
