...
async def force_subscribe(client, message):
    ...
    if toFsub == message.chat.id:
        return await m.edit(
            "It seems like you're attempting to enable force subscription for this chat ID. Please use a different chat ID !"
        )
    if not await is_check_admin(client, toFsub, client.me.id):
        return await m.edit(
            "I need to be an admin in the given chat to perform this action!\nMake me admin in your Target chat and try again."
        )
    try:
        await db.setFsub(grpID=message.chat.id, fsubID=toFsub)
        return await m.edit(
            f"Successfully added force subscribe to {toFsub} in {message.chat.title}"
        )
    except Exception as e:
        logger.exception(e)
        return await m.edit(
            f"Something went wrong ! Try again later or report in Support Group @Jisshu_support"
        )
...
async def del_force_subscribe(client, message):
    ...
    if ifDeleted:
        return await m.edit(
            f"Successfully removed force subscribe for - {message.chat.title}\nTo add again use <code>/fsub YOUR_FSUB_CHAT_ID</code>"
        )
    else:
...
async def show_fsub(client, message):
    ...
    if fsub:
        ...
        await m.edit(
            f"Force subscribe is set to {fsub}\n<a href={invite_link}>Channel Link Link</a>",
