    ...
    try:
        ...
        if "#" not in command_args or len(command_args.split("#")) < 3:
            await message.reply_text(
                f"Usage: /set_ads {{ads name}}#{{time}}#{{photo URL}} <a href=https://t.me/Movie_Links_Hub/9>Explain</a>"
            )
            return
        ...
        if len(ads_name) > 35:
            await message.reply_text(
                f"Advertisement name should not exceed 35 characters."
            )
            return
        ...
        if duration_or_impression[0] == "d":
            ...
            await message.reply_text(
                f"Invalid prefix. Use 'd' for duration and 'i' for impression count."
            )
            return
        ...
        if not reply:
            await message.reply_text(
                f"Reply to a message to set it as your advertisement."
            )
            return
        if not reply.text:
